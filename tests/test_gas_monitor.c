...
static void test_buzzer_value(void)
{
    ...
    gas_config_defaults(&c);
    assert(c.buzzer == 5u && gas_config_valid(&c));
    ...
    assert(gas_buzzer_duration_ms(&c) == 0u);
    c.buzzer = 1u;
    assert(gas_buzzer_duration_ms(&c) == 1000u);
    c.buzzer = 60u;
    assert(gas_buzzer_duration_ms(&c) == 60000u);
    c.buzzer = GAS_BUZZER_ALWAYS;
    assert(gas_buzzer_duration_ms(&c) == GAS_BUZZER_FOREVER_MS);
    ...
    gas_buzzer_name(GAS_BUZZER_OFF, name, sizeof(name));
    assert(strcmp(name, "OFF") == 0);
    gas_buzzer_name(1u, name, sizeof(name));
    assert(strcmp(name, "1S") == 0);
    gas_buzzer_name(60u, name, sizeof(name));
    assert(strcmp(name, "60S") == 0);
    gas_buzzer_name(GAS_BUZZER_ALWAYS, name, sizeof(name));
    assert(strcmp(name, "ALWAYS") == 0);
    /* 越界同样读作最长档，而不是印出 "66S" 这种根本不存在的设置。 */
    gas_buzzer_name(66u, name, sizeof(name));
    assert(strcmp(name, "ALWAYS") == 0);

    /* 缓冲区不够时截断，不越界；大小为 0 时一个字节都不写。 */
    tiny[0] = (char)0x7f;
    tiny[1] = (char)0x7f;
    gas_buzzer_name(60u, tiny, sizeof(tiny));
    assert(tiny[0] == '6' && tiny[1] == '\0');
    name[0] = 'X';
    gas_buzzer_name(GAS_BUZZER_OFF, name, 0u);
    assert(name[0] == 'X');
}
...
static void test_buzzer_keys(void)
{
    ...
    unsigned i;
    run_to_normal(&m, 0);
    for (i = 0; i < GAS_SEL_BUZZER; ++i) gas_monitor_key(&m, 1, 63000);
    ...
    for (i = 0; i < GAS_BUZZER_MAX_S - 2u; ++i) gas_monitor_key(&m, 2, 63000);
...
static void test_buzzer_set(void)
{
    ...
    run_to_normal(&m, 0);
    /* 60 以内逐档都对，61 是「一直响」。 */
    assert(gas_monitor_set_buzzer(&m, 1u, 63000) && m.config.buzzer == 1u);
    assert(gas_monitor_set_buzzer(&m, GAS_BUZZER_MAX_S, 63000));
    assert(m.config.buzzer == GAS_BUZZER_MAX_S);
    assert(gas_monitor_set_buzzer(&m, GAS_BUZZER_ALWAYS, 63000));
    assert(m.config.buzzer == GAS_BUZZER_ALWAYS);
    assert(gas_monitor_set_buzzer(&m, GAS_BUZZER_OFF, 63000));
    assert(m.config.buzzer == GAS_BUZZER_OFF && gas_buzzer_duration_ms(&m.config) == 0u);

    /* 超出编码范围的值必须在收窄成 uint8 之前就被拒：256 截断成 0 会成为
     * 一个合法取值，静默地把蜂鸣器关掉，而调用者要的显然不是这个。 */
    assert(!gas_monitor_set_buzzer(&m, GAS_BUZZER_ALWAYS + 1u, 63000));
    assert(!gas_monitor_set_buzzer(&m, 256u, 63000));
    assert(!gas_monitor_set_buzzer(&m, 0xffffu, 63000));
    assert(m.config.buzzer == GAS_BUZZER_OFF);

    /* 面板上显示的名字就是串口回显的名字，两边共用一份拼写。 */
    gas_buzzer_name(m.config.buzzer, name, sizeof(name));
    assert(strcmp(name, "OFF") == 0);
     ...
     * 就必须重启。 */
    gas_monitor_key(&m, 1, 63000);
    for (i = 0; i < 100u; ++i) gas_monitor_key(&m, 3, 63000);
    for (t = 63100; t <= 66100; t += 100) sample(&m, t, 100, 100, 100);
    assert(m.state == GAS_SAFE_WAIT && m.reset_ready);

    m.dirty = false;
    assert(gas_monitor_set_buzzer(&m, 30u, 66101));
    assert(m.dirty && m.reset_ready);
    assert(gas_monitor_set_threshold(&m, GAS_MQ4, 2600u, 66101));
    assert(!m.reset_ready);
    /* 窗口从改动那一刻重新开始计时，之后照常成立。 */
    for (t = 66200; t <= 69200; t += 100) sample(&m, t, 100, 100, 100);
    assert(m.reset_ready);
    gas_monitor_key(&m, 4, 69200);
    assert(gas_monitor_valve_open(&m));
}
...
int main(void)
{
    ...
    test_keys();
    test_sample_period();
    puts("PASS: gas_monitor states/power-up/fault/recovery/keys/period/buzzer/wraparound");
    return 0;
}
