...
已用 STM32CubeMX 生成 STM32F103C8T6 HAL 工程，配置为 `stm32f103/cubemx/smart_gas_monitor.ioc`。已实现三路采样、报警锁存、KEY4 安全恢复、按键阈值调整及 AT24C02 参数保存。OLED 显示、串口命令和报警历史持久化尚未实现。

硬件空目录通过 `.gitkeep` 纳入 Git 管理。固件应用分层如下：

```text
stm32f103/app/
├── monitor/        # 阈值、监测状态机、报警锁存和人工恢复
├── parameters/     # 参数序列化、CRC、双副本保存
└── runtime/        # 主循环调度与模块装配
```

`cubemx/Core/`、`cubemx/Drivers/` 由 CubeMX 生成；`bsp/` 访问 HAL，不依赖 app。根目录 `tests/` 保存主机 C 测试。
...
固件输出在 `stm32f103/cubemx/build/`。`cubemx/GNUmakefile` 加入外层 app/bsp 源码，CubeMX 可重新生成其管理的 `Makefile`。主机测试编译命令见固件说明。
