...
| OLED 显示 | 部分 | 曾在 I2C1 走 PB8/PB9 时验证：读回 1024 字节帧缓冲与字模表逐格比对位差为 0，四个页面内容均正确，`oled.ready = true` 说明面板真实应答。**现已把 I2C1 改到默认引脚 PB6/PB7**（Proteus 的 STM32 模型不认引脚重映射），实物接线需相应改到 PB6/PB7 后复验 |
