...
- 报警时蜂鸣器按 TIM2 的 500 个 10 ms 节拍响 5 秒后自动停（`BUZZER_ALARM_TICKS`），红灯和关阀保持不变。时间固定不可配；持续报警不重新计时，只有新的报警才重新响。
...
配置占 0x00～0x0F 和 0x10～0x1F 两个 16 字节槽。v4 布局为：0 magic、1 version、2～3 sequence、4～9 三路阈值、10～11 采样周期、12 lockout、13～14 CRC16-CCITT（覆盖 0～12）、15 commit。先清除另一槽提交标志，再写正文，最后提交并读回校验。底层按 8 字节页边界拆分并 ACK polling。

版本号当前为 4。旧版本记录会被判为无效并回退默认配置，避免按新布局误读；加载时验证 CRC、版本、阈值与采样周期范围，选择有效且较新的副本。lockout 字节纳入 CRC，单字节损坏不能改变启动时的安全锁存而仍通过校验。
