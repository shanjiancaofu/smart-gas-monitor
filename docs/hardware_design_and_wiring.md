...
| 执行与报警 | 排风扇继电器、舵机阀门、蜂鸣器、绿黄红 LED、阀门 LED | 报警时关阀断气并开风扇排风，锁存待人工确认 |
...
- MQ-4、MQ-6、MQ-7 加热器使用 5 V，三路共地。5 V 电源建议至少 **2 A**：MQ 加热器三路合计约 0.5 A，舵机堵转可到 1 A 以上，继电器和 HC-05 再占一些。
...
| **5 V** | MQ 加热器、继电器模块 DC+、舵机、HC-05、USB-TTL |
...
```text
舵机 VCC（红） ── 独立 5 V 电源正      ← 不要从 STM32 的 3.3 V 取
舵机 GND（棕） ── 电源地（与 MCU 共地）
舵机 信号（橙）── PB8                  ← TIM4_CH3，50 Hz PWM
```
...
状态、风扇、舵机阀门与灯/蜂鸣器的完整对应关系见[硬件接口：状态与执行器映射](hardware_interface.md#状态与执行器映射)。PA8 继电器驱动风扇，PB8 舵机控制燃气阀门；普通预热不开风扇，恢复 EEPROM lockout 时保持排风和关阀。锁存在掉电后如何恢复（含三条已知限制）见[锁存与掉电恢复](hardware_interface.md#锁存与掉电恢复)。
