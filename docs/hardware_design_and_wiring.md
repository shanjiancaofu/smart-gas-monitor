...
逻辑：`PA8 高 → 继电器吸合 → COM-NO 导通 → 风扇转`；`PA8 低 → 释放 → 风扇停`。PA8 建议加 10 kΩ 下拉，保证复位期间风扇关闭。燃气阀门由 PB8 舵机独立控制；舵机断电后的实际位置不能由 PA8 电平推断。
...
状态、风扇、舵机阀门与灯/蜂鸣器的完整对应关系见[硬件接口：状态与执行器映射](hardware_interface.md#状态与执行器映射)。PA8 继电器驱动风扇，PB8 舵机控制燃气阀门；普通预热不开风扇，恢复 EEPROM lockout 时保持排风和关阀。
