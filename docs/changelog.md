...
- 配置格式升级到 v5：sequence 由 2 字节压成 1 字节，腾出的 byte 3 给蜂鸣器时长，其余偏移与 CRC 覆盖长度都与 v4 一致。加字段是必须的——两个 16 字节槽都已排满，历史区 0x20～0xFF 又被 14 条记录正好占满，整片 AT24C02 没有空余字节。sequence 只用于判断两槽新旧、逐次 +1 递增、按模比较，8 位与 16 位结果相同。**v4 及更早的记录会被判为无效并回退默认配置**，阈值和采样周期也一并回落——这是有意为之，见下。
...
- 串口报警通知复用 `app_run()` 里已有的报警边沿判断，与写历史记录共用同一处 `last_state` 比较，不新增第二份「什么是新报警」的判据。
...
- 命令集：`STATUS?`、`CONFIG?`、`HISTORY?`、`SET MQ4|MQ7|MQ8 <值>`、`SET PERIOD <毫秒>`、`VALVE CLOSE`，行以 `\r\n` 结束，大小写不敏感。报警开始时在两路主动推一行 `ALARM`。
...
- 历史页上的 KEY2/KEY3 改为翻阅记录，在 `app_run()` 的按键分发里截获，不进入参数处理逻辑——历史页没有可调项，否则这两个键会去改最后选中的那个参数。
- `app/history/history.*`：0x20～0xFF 的 14 条 16 字节环形记录，含 16 位序号、上电秒数、三路 ADC 值、报警通道掩码和 CRC16。序号由记录本身携带并在启动时扫描得出，重启后接着往下编。

### 变更

- `app_run()` 增加报警边沿检测：只在**首次进入 ALARM** 时写一条记录。状态机决定报警何时开始，上一次的状态用来区分「新报警」和「报警还在持续」，后者不重复写。
