...
日期：最新一条为 2026-09-18；`841bcc3` 及其以下各条为 2026-09-16，顶部目录重构一条为 2026-09-15，其余各条为 2026-09-14。

> **`6777b86`（2026-09-17 上午）到 `33af800` 之间的条目未补**，那一段的改动记在各自的提交信息里。下面的记录从 `841bcc3` 起仍是完整的。

## `838100c` — 仿真的舵机改走 PA1，实物不受影响

**Proteus 的 STM32 模型不实现 TIM4_CH3 走 PB8。** PB8 上量不到任何波形，舵机在仿真里一直停在原位。它和 `12eeaf8` 那条「不认 I2C1 引脚重映射」是同类的模型缺失——**不报错、不提示，只表现为「引脚没波形」**。两份能跑的参考工程用的都是 TIM2_CH2 / PA1，而 TIM2 在这个仿真里是好的：工程自己的 10 ms 系统节拍一直跑在它上面。

排查走过弯路。先后试过 TIM4_CH3 硬件 AF、TIM4 中断软件翻转、TIM2 中断软件翻转三种做法，全都没用；其中还夹着一个我自己的 bug——把 TIM2 的比较通道留在复位默认的「冻结」模式，参考手册写明该模式下比较结果「对输出没有影响」，实测连 `CC1IF` 都不置位，于是 PB8 拉高之后再没人拉低，舵机被钉死在最远端。真正把方向掰正的是一次 **1 Hz 方波诊断**：让 PB8 慢速翻转，肉眼就能看网络指示点，一次证明了驱动和接线都是好的。教训是——**引脚能不能驱动、线有没有接上，这种前提要单独验掉再动架构**，别把没验证过的前提当成下一次修改的基础。

**最终做法：仿真与实物用不同的引脚**，差异全部收在 `#if USE_SOFT_I2C` 里，实物的那条路径一行未动。
...
- PA1 让给舵机后 MQ6 必须搬走。C8 是 48 脚封装、**没有 PC0~PC5**，十个带 ADC 的引脚（PA0~PA7、PB0/PB1）全被占满，所以还得再挤一个器件出来：MQ6 接 PA5，而 PA5 原本的阀门指示灯改到 PB9。挑阀门灯是因为它最不显眼——报警时红灯和风扇已经足够提示，阀门状态在 OLED 上也有文字。
- 新增 `bsp_led_init()`：仿真分支的阀门灯不在 `.ioc` 里，`MX_GPIO_Init()` 不会配它。实物分支是空实现。
- 仿真分支把 TIM2 的周期从 10 ms 改成 20 ms（50 Hz 是舵机的硬要求），系统节拍由 `stm32f1xx_it.c` 的 TIM2 中断里补第二次 `app_tick_isr()` 维持。`app.c` 里所有业务计时走的是 `HAL_GetTick()`，不受影响；副作用只有蜂鸣器节奏的时间分辨率降到 20 ms，而最短的一段是 80 ms，听感不变。
- **`HAL_TIM_PWM_Start()` 在这个位置不能用**：它会把 `htim2.State` 置成 BUSY，而 `app_init()` 紧接着要调`HAL_TIM_Base_Start_IT(&htim2)` 起系统节拍，那个函数一看到 `State != READY` 就返回 `HAL_ERROR`，一路走到 `Error_Handler`。改用 `TIM_CCxChannelCmd()` 只开通道输出，计数器留给 `Base_Start_IT` 去启动。
- 仿真分支的引脚差异全部配在 BSP 里，**没有改 `tim.c` / `main.h` / `.ioc`**：`.ioc` 是 `Core/` 的唯一真源，动它就得跑 `tools/generate_hal.py` 重生成，而那会连带重写实物分支的 `Core/`，和「实物不动」直接冲突。

同一笔提交里还带了以下几项独立修复：

- **蜂鸣器档位重编码**为 `-1=ALWAYS / 0=OFF / 1..60=秒`，并且首尾相接成环——原来 `ALWAYS` 和 `OFF` 在数轴上是挨着的，逐档走会得到反直觉的顺序。配置格式升到 **v8**，`v6`/`v7` 记录仍能就地升级读取（阈值、周期，尤其是落盘的 `lockout` 原样保留）。
- **恢复线定死**为 WARNING 80% / SAFE 70% / RELEASE 75%，和硬件文档一直以来的说法对齐。
- **换页和翻历史记录时整屏推**。此前只推一个 page，一屏要八帧、约 0.7 秒才铺完，中间新旧两页并存，屏幕上就是花的。稳态仍是一帧一页。另外历史记录加了缓存，不再每次重画（每 200 ms）都从 EEPROM 回读一遍——那是全工程唯一在重画路径上走 I2C 的页面。
- **软件 I2C 接管引脚前先 `HAL_I2C_DeInit()`**，否则硬件 I2C 还使能着，总线上的边沿会被它当成起始条件去响应。
- **UART 的 ErrorCallback 把 `HAL_UART_Receive_IT()` 的返回值写回 `ready`**。此前直接丢弃，重启失败时端口看起来仍是好的。
- **`tools/build_keil.ps1` 不再把 UV4 的警告（退出码 1）当失败**。原来一旦有警告就在收集产物前抛异常，`Artifacts/` 里留着上一版 HEX，Proteus 会静默地继续跑旧固件——「编译过了但仿真里还是旧行为」极难查。
- `tools/serial_check.py` 的期望值跟着新的蜂鸣器编码更新。
