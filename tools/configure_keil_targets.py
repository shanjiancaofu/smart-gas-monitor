...
def configure(project: pathlib.Path) -> None:
    ...
    for mode, value in (("hw", "0"), ("soft", "1")):
        ...
        common.find("OutputDirectory").text = f"..\\..\\build\\keil5\\{mode}\\Objects\\"
        common.find("OutputName").text = f"smart_gas_monitor_keil_{mode}"
        common.find("ListingPath").text = f"..\\..\\build\\keil5\\{mode}\\Listings\\"
        target.find(".//Cads/VariousControls/Define").text = f"USE_HAL_DRIVER,STM32F103xB,USE_SOFT_I2C={value}"
        after = common.find("AfterMake")
        after.find("RunUserProg1").text = "0"
        after.find("RunUserProg2").text = "0"
...
if __name__ == "__main__":
    configure(pathlib.Path(__file__).resolve().parents[1] / "stm32f103/MDK-ARM/smart_gas_monitor.uvprojx")
